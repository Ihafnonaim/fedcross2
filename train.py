...
def initialization():
    ...
    nodes = []
    prev_models = {} # Store previous models for contrastive loss
    server_model = None 
    val_fold = None
    test_fold = None
    weight_sum = 0
    for node_id, [node_name, d_name, d_path, fraction] in enumerate(cfg['node_list']):
        ...
        prev_models[node_name] = local_model.state_dict()

        if server_model is None:
            server_model = nn.DataParallel(module=UNet(in_ch=1, base_ch=32, cls_num=cfg['cls_num'])) 
    ...
    return nodes, train_record, dl_val, dl_test, prev_models, server_model
...
def train_local_model(local_model, prev_model, server_model, optimizer, scheduler, data_loader, epoch_num):
    train_loss = 0
    train_loss_num = 0

    prev_model.load_state_dict(prev_model.state_dict())
    server_model.eval()
    ...
    for epoch_id in range(epoch_num):
        ...
        for batch in data_loader:
            ...
            pred, pred_logit, present_embed = local_model(image)
            _, _, serv_embed = server_model(image)
            _, _, prev_embed = prev_model(image)
            ...
            # Contrastive loss
            loss_con = contrastive_loss(prev_embed, present_embed, serv_embed)

            loss_sup += loss_con
            epoch_loss += l_dice.item() + l_ce.item() + loss_con.item()
            epoch_loss_num += 1

            print_line += ' --- Loss: {0:.6f}({1:.6f}/{2:.6f}/{3:.6f})'.format(loss_sup.item(), l_dice.item(), l_ce.item(), loss_con.item())
            print(print_line)
...
def train():
    ...
    nodes, train_record, dl_val, dl_test, prev_models, server_model = initialization()
    ...
    for commu_t in range(start_iter, cfg['commu_times'], 1):
        ...
        for i, [local_model, optimizer, scheduler, _, _, _] in enumerate(nodes):
            ...
            prev_model = nn.DataParallel(module=UNet(in_ch=1, base_ch=32, cls_num=cfg['cls_num']))
            prev_model.load_state_dict(prev_models[node_name])
            prev_model.cuda()

            local_model.train()

            train_loss.append(train_local_model(local_model, prev_model, server_model, optimizer, scheduler, dl_train, cfg['epoch_per_commu']))
